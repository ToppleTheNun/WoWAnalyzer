...
class VanishFindWeakness extends Analyzer {
  ...
  constructor(options: Options) {
    ...
    this.addEventListener(
      Events.cast.by(SELECTED_PLAYER).spell(SPELLS.SHADOWSTRIKE),
      this.onFindWeaknessApplierCast,
    );
    this.addEventListener(
      Events.cast.by(SELECTED_PLAYER).spell(SPELLS.CHEAP_SHOT),
      this.onFindWeaknessApplierCast,
    );
  }
  ...
  onFindWeaknessApplierCast(event: CastEvent): void {
    ...
    const enemyHasFW: boolean = enemy.hasBuff(
      SPELLS.FIND_WEAKNESS.id,
      event.timestamp,
      0,
      0,
    ...
    if (!enemyHasFW) {
      ...
      const fwTimeRemaining: number = enemy.getRemainingBuffTimeAtTimestamp(
        SPELLS.FIND_WEAKNESS.id,
        this.FW_DEBUFF_LENGTH,
        this.DEBUFF_CAP,
        event.timestamp,
        this.selectedCombatant.id,
      );
